...
GPS_ASSET_URL = os.environ.get("GPS_ASSET_URL", "https://fleetmanagement-api-clust03.gpscockpit.com/api/asset")
GPS_PERMANENT_TOKEN = os.environ.get("GPS_PERMANENT_TOKEN", os.environ.get("GPS_TOKEN", ""))
...
def get_gps_locations():
    ...
    # 1. Fetch Bearer Token using the Refresh Token
    auth_url = "https://fleetmanagement-api-clust03.gpscockpit.com/api/Authentication/RefreshToken"
    try:
        auth_resp = requests.post(auth_url, json={"refreshToken": token}, timeout=10)
        if auth_resp.status_code != 200:
            return jsonify({
                "error": "فشل المصادقة مع GPS Cockpit",
                "status_code": auth_resp.status_code,
                "response": auth_resp.text[:200]
            }), 401
        
        bearer_jwt = auth_resp.json().get("token")
        if not bearer_jwt:
            return jsonify({"error": "الرد لم يحتوي على JWT"}), 401
    except requests.Timeout:
        return jsonify({"error": "تجاوز وقت استجابة المصادقة"}), 504
    except Exception as e:
        return jsonify({"error": "حدث خطأ أثناء المصادقة", "details": str(e)}), 500

    # 2. Fetch Assets using the Bearer Token
    headers = {
...
def index():
    ...
    b64_en = load_logo()
    return render_template("index.html", google_user=google_user, b64_en=b64_en, gps_token=get_gps_token())
...
def oils():
    google_user = session.get("google_user")
    b64_en = load_logo()
    return render_template("oils.html", google_user=google_user, b64_en=b64_en, gps_token=get_gps_token())
...
def purchase():
    google_user = session.get("google_user")
    b64_en = load_logo()
    return render_template("purchase.html", google_user=google_user, b64_en=b64_en, gps_token=get_gps_token())
...
def schedule():
    google_user = session.get("google_user")
    b64_en = load_logo()
    return render_template("schedule.html", google_user=google_user, b64_en=b64_en, gps_token=get_gps_token())
...
def washing():
    google_user = session.get("google_user")
    b64_en = load_logo()
    return render_template("washing.html", google_user=google_user, b64_en=b64_en, gps_token=get_gps_token())
...
def gps_sync():
    google_user = session.get("google_user")
    b64_en = load_logo()
    return render_template("gps_sync.html", google_user=google_user, b64_en=b64_en, gps_token=get_gps_token())
