...
for html_file in ['templates/login.html', 'templates/password.html']:
    path = os.path.join(r'c:\Users\Administrator.MADEBYA-NBLPM4O\Desktop\Invoice-Bin-zoma-main32232', html_file)
    with open(path, 'r', encoding='utf-8') as f:
        html = f.read()

    if 'bz.sa' not in html:
        html = html.replace('<body>', '<body>\n' + navbar_html)
    
    if 'site-logo' not in html:
        html = html.replace('<div class="login-card glass-surface">', '<div class="login-card glass-surface">\n' + logo_html)

    with open(path, 'w', encoding='utf-8') as f:
        f.write(html)

print("Added navbar to auth pages")
