...
def api_quick_search():
    ...
    try:
        with db_connection() as db:
            c = db.execute("SELECT id, name, empid, plate, car, iqama, status FROM drivers WHERE name LIKE ? OR empid LIKE ? OR plate LIKE ? OR iqama LIKE ? LIMIT 10",
                           (f"%{q}%", f"%{q}%", f"%{q}%", f"%{q}%"))
            for r in c.fetchall():
                results.append({
                    "id": r["id"],
                    "title": r["name"],
                    "subtitle": f"رقم وظيفي: {r['empid']} | لوحة: {r['plate']}",
                    "type": "driver",
                    "url": f"/registry?tab=drivers&search={r['plate']}"
                })
            
            c = db.execute("SELECT id, name, empid, plate, iqama, job FROM hr_employees WHERE name LIKE ? OR empid LIKE ? OR plate LIKE ? OR iqama LIKE ? LIMIT 10",
                           (f"%{q}%", f"%{q}%", f"%{q}%", f"%{q}%"))
            for r in c.fetchall():
                if not any(res["type"] == "driver" and (res.get("title") == r["name"] or str(r['empid']) in res.get("subtitle", "")) for res in results):
                    results.append({
                        "id": f"hr_{r['id']}",
                        "title": r["name"],
                        "subtitle": f"موظف (HR) - {r['job'] or 'N/A'} | لوحة: {r['plate']}",
                        "type": "hr_employee",
                        "url": f"/registry?tab=drivers&search={r['empid']}"
                    })
    except Exception as e:
...
@login_required
def about_page():
    return render_template("info_page.html", page_title="من نحن", eyebrow="هوية بن زومة",
        description="مركز تشغيل رقمي موحد يساعد فرق شركة بن زومة الدولية على متابعة الأسطول والأفراد والسجلات من مساحة آمنة واحدة.",
        cards=[
            {"title": "مهمة واضحة", "text": "تحويل البيانات التشغيلية إلى قرارات سريعة وقابلة للمتابعة."},
            {"title": "مصدر موحد", "text": "تقليل ازدواج البيانات وربط السائقين والمركبات والتنبيهات."},
            {"title": "خصوصية وصلاحيات", "text": "عرض كل مساحة للمستخدم المصرح له فقط، دون تغيير البيانات."},
        ])
...
def how_it_works_page():
    return render_template("info_page.html", page_title="كيف يعمل النظام", eyebrow="مسار عمل مختصر",
        description="ابدأ من مركز القيادة، اختر مساحة العمل، راجع المؤشر أو السجل، ثم نفّذ الإجراء المطلوب مع حفظ أثر العملية.",
        cards=[
            {"title": "01 · راقب", "text": "اقرأ المؤشرات والتنبيهات ذات الأولوية من لوحة القيادة."},
            {"title": "02 · انتقل", "text": "استخدم المشغل الموحد أو القائمة للوصول إلى مساحة العمل."},
            {"title": "03 · نفّذ ووثّق", "text": "حدّث السجل المطلوب، ثم راجع النتيجة في سجل النشاط."},
        ])
...
@login_required
def faq_page():
    return render_template("info_page.html", page_title="الأسئلة الشائعة", eyebrow="إجابات سريعة",
        description="إرشادات مختصرة لأكثر الأسئلة المتعلقة بالدخول والصلاحيات وحفظ البيانات واستخدام النظام على الجوال.",
        cards=[
            {"title": "لماذا لا أرى قسمًا معينًا؟", "text": "تظهر الأقسام حسب دور المستخدم وصلاحيات التبويبات التي يحددها المدير."},
            {"title": "هل التعديلات تحفظ تلقائيًا؟", "text": "يعتمد ذلك على مساحة العمل؛ تظهر رسالة نجاح بعد عمليات الحفظ المعتمدة."},
            {"title": "هل يعمل على الجوال؟", "text": "نعم، الواجهة متجاوبة مع أهداف لمس واضحة وتمرير آمن للجداول."},
        ])
...
def _collect_expiry_alerts(window_days=90, rid=None):
    ...
    for d in db_drivers:
        if d.phone:
            ...
            active_custody = next((c for c in d.custodies if c.status == 'active'), None)
            if active_custody and active_custody.vehicle and active_custody.vehicle.plate_number:
                phone_map[active_custody.vehicle.plate_number] = d.phone
...
@app.route("/api/fleet_summary", methods=["GET"])
@login_required
def api_fleet_summary():
    """Quick summary of the fleet status for reports."""
    try:
        with db_connection() as db:
            c = db.execute("SELECT status, COUNT(*) as count FROM drivers GROUP BY status")
            stats = {row["status"]: row["count"] for row in c.fetchall()}
            active = stats.get("نشط", 0) + stats.get("متاح", 0)
            inactive = sum(v for k, v in stats.items() if k not in ["نشط", "متاح"])
            return jsonify({
                "active": active,
                "inactive": inactive,
...
def api_sync_excel():
    try:
        ...
        # Fill missing vehicle document dates from merged Excel (Hijri-aware); no overwrite of existing dates
        vehicle_docs = {"success": False}
        try:
            from utils.update_from_excel import fill_missing_vehicle_docs
            vehicle_docs = fill_missing_vehicle_docs()
        except Exception as _vd_err:
            vehicle_docs = {"success": False, "error": str(_vd_err)}

        return jsonify({
            "success": True,
            "message": "تم تحديث البيانات من ملف الإكسيل بنجاح",
    ...
    try:
        ...
        for d in drivers:
            ...
            results.append({
                ...
                "job": d.get("job", ""),
                "iqama_expiry": d.get("iqama_expiry", ""),
                "passport_expiry": d.get("passport_expiry", ""),
