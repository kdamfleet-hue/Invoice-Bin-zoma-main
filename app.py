...
DEFAULT_TEMPLATES = {
    ...
    "workshop": os.path.join("static", "تقرير الورشة.xlsx"),
}
...
def download_workshop_template():
    ...
    template_path = os.path.join(os.path.dirname(__file__), "static", "تقرير الورشة.xlsx")
...
def generate_washing():
    try:
        ...
        template_path = os.path.join(app.root_path, "static", "washing_template.xlsx")
        if not os.path.exists(template_path):
            return (
        ...
        logo_path = os.path.join(
            app.root_path, "static", "excel_logo.png"
        )
        if os.path.exists(logo_path):
            img = XLImage(logo_path)
            img.width = 300
