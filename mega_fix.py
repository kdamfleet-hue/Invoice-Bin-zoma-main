...
print("1. Updating Database from vehicle_lookup_v2.xlsx")
try:
...
print("2. Fixing generate_washing in app.py")
with open('app.py', 'r', encoding='utf-8') as f:
    app_code = f.read()

# Replace ws.cell(row=row, column=3).value = ... with write_unmerged wrapper in generate_washing
washing_fix = """
...
if 'def write_unmerged' not in app_code:
    ...
    # Also fix export_po to read odometer
...
print("3. Updating HTML Templates")
whatsapp_btn = '''
<a href="https://wa.me/?text=مرحباً" target="_blank" style="position: fixed; bottom: 20px; right: 20px; z-index: 999; background-color: #25d366; color: white; border-radius: 50px; padding: 15px 25px; box-shadow: 0 4px 10px rgba(0,0,0,0.2); font-weight: bold; text-decoration: none; display: flex; align-items: center; gap: 10px; transition: 0.3s; font-family: Cairo;">
...
'''
logout_btn = '''
<a href="/logout" style="position: absolute; top: 1.5rem; left: 1.5rem; z-index: 100; background: #e74c3c; color: white; padding: 8px 20px; border-radius: 50px; font-weight: bold; font-size: 0.95rem; text-decoration: none; font-family: Cairo;">تسجيل الخروج</a>
'''
...
for f_path in glob.glob('templates/*.html'):
    ...
    # Logo fix
    html = re.sub(r'<img[^>]*src="\{\{\s*url_for\(\'static\',\s*filename=\'new_logo_clean\.png\'\)\s*\}\}"[^>]*>', 
                  '<img src="{{ url_for(\'static\', filename=\'site_logo.jpg\') }}" alt="شعار بن زومة" class="logo-img" style="max-width:250px; object-fit:contain;">', html)
    html = re.sub(r'\{\%\s*if\s*b64_en\s*\%\}.*?\{\%\s*endif\s*\%\}', 
                  '<img src="{{ url_for(\'static\', filename=\'site_logo.jpg\') }}" alt="Logo" class="site-logo" style="width: 300px; max-width: 90%; height: auto; object-fit: contain; margin: 0 auto 1.5rem auto; display: block; filter: drop-shadow(0 4px 6px rgba(0,0,0,0.1));">', html, flags=re.DOTALL)

    # Logout btn
    if 'تسجيل الخروج' not in html and 'login' not in f_path and 'password' not in f_path:
        html = html.replace('<body>', '<body>\n' + logout_btn)

    # WhatsApp btn
    if 'واتساب' not in html and 'login' not in f_path and 'password' not in f_path:
        html = html.replace('</body>', whatsapp_btn + '\n</body>')

    # Purchase specifics
    if 'purchase.html' in f_path:
        # Add odometer input
        if 'id="odometer"' not in html:
            html = html.replace('<div class="form-group"><label>رقم الإقامة</label>', 
                                '<div class="form-group"><label>عداد السيارة (كم)</label><input type="number" id="odometer" placeholder="مثال: 150000" required></div><div class="form-group"><label>رقم الإقامة</label>')
        # Add odometer to downloadPO payload
        ...
        # Add odometer to summary if not already there
        if 'عداد السيارة' not in html.split('let summaryHtml')[1][:500]:
            html = html.replace('<strong>التاريخ:</strong> ${document.getElementById(\'poDate\').value}</p>',
                                '<strong>التاريخ:</strong> ${document.getElementById(\'poDate\').value}</p><p><strong>عداد السيارة:</strong> ${document.getElementById(\'odometer\').value}</p>')

    # Schedule print button fix
    if 'schedule.html' in f_path:
        html = html.replace('onclick="window.print()"', 'onclick="exportExcel()"')

    with open(f_path, 'w', encoding='utf-8') as f:
        f.write(html)

print("4. Modifying po_template.xlsx to add Odometer field")
try:
    ...
    print("Error modifying po_template.xlsx:", e)
