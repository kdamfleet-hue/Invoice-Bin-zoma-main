...
def upgrade():
    ...
    if dialect == 'postgresql':
        for t in tables_with_fk:
            op.execute(f"ALTER TABLE IF EXISTS {t} ADD COLUMN IF NOT EXISTS branch_id INTEGER REFERENCES erp_branches(id);")
        op.execute("ALTER TABLE IF EXISTS erp_settings ADD COLUMN IF NOT EXISTS branch_id INTEGER;")
    else:
        for t in tables_with_fk:
            try:
