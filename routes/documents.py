...
from flask import Blueprint, render_template, session, request, jsonify, send_file, current_app, g, redirect, url_for
import threading
from helpers import login_required, load_logo, blob_get, blob_set, audit_and_verify, current_branch_id, _audit_add
from models.schema import db, Driver, Document, AuditLog
from helpers import DOC_TYPES, _alert_cfg, _alert_cfg_set, ALERT_CRON_KEY, ALERT_RECIPIENTS
from routes.schedule import _harvest_driver_registry, _harvest_vehicle_registry, _norm_iqama
...
# Upload limits — kept in sync with the equivalent (shadowed-by-registration-order,
# otherwise-unreachable) constants in routes/api_docs.py.
...
def api_alert_settings():
    from app import _send_scheduled_digest
    if not session.get("is_admin"):
...
def alerts_center_update():
    from app import _alerts_center_clean_value, _documents_rows
    try:
        body = request.get_json(silent=True) or {}
        src = body.get("src")
