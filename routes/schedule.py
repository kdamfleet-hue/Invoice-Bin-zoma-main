...
@schedule_bp.route("/washing_report")
@role_required('admin', 'operations', 'maintenance')
def washing_report():
    """تقرير محطة الغسيل الشهري — يُحسب مباشرة من blob_get"""
    ...
    WASH_PRICE_MAP = {"lorry": 80, "medium": 75, "small": 30}

    def wash_category(vtype):
        t = str(vtype or "").lower()
        if any(x in t for x in ["بيك اب","بيكب","بيك أب","pickup","pick up"]): return "small"
        if any(x in t for x in ["دينا","صندوق","جيب","فورتشنر","fortuner","برادو","prado"]): return "medium"
        if any(x in t for x in ["ايسوزو","إيسوزو","شاحنة","لوري","لواري","قلاب","isuzu","truck"]): return "lorry"
        return "small"

    def wash_price(v):
        if v.get("price") not in (None, "", "None"):
            try: return float(v["price"])
            except: pass
        return WASH_PRICE_MAP[wash_category(v.get("type",""))]

    now = datetime.now()
    current_month = now.month - 1   # 0-based
    current_month_name = MONTH_NAMES[current_month]

    vehicles_raw = blob_get("washing_schedule") or []
    vehicles = vehicles_raw if isinstance(vehicles_raw, list) else []

    # Ensure every vehicle has a valid 12-month array
    for v in vehicles:
        if not isinstance(v.get("m"), list) or len(v.get("m", [])) != 12:
            v["m"] = [0] * 12

    # Duplicate plate detection and automatic deduplication
    ...
    for v in vehicles:
        ...
        if not plate:
            plate = f"__no_plate_{id(v)}"
            
        if plate in merged_vehicles:
            deduplicated_count += 1
            existing = merged_vehicles[plate]
            # Merge wash counts for all 12 months
            for i in range(12):
                existing["m"][i] += v["m"][i]
            # Merge driver if missing in existing
            if (not existing.get("driver") or existing.get("driver") == "غير محدد") and v.get("driver"):
    ...
    current_washes = sum(1 for v in vehicles if v["m"][current_month] > 0)
    not_washed     = total_vehicles - current_washes

    # Finance
    total_amt  = sum(wash_price(v) * sum(v["m"]) for v in vehicles)
    month_amt  = sum(wash_price(v) for v in vehicles)
    paid_amt   = sum(wash_price(v) for v in vehicles if v["m"][current_month] > 0)
    pend_amt   = month_amt - paid_amt
    ...
    return render_template(
        ...
        current_washes=current_washes,
        not_washed=not_washed,
        total_amt=f"{total_amt:,.0f}",
        month_amt=f"{month_amt:,.0f}",
        paid_amt=f"{paid_amt:,.0f}",
        pend_amt=f"{pend_amt:,.0f}",
        month_name=current_month_name,
        current_month=current_month,
        report_date=now.strftime("%Y/%m/%d"),
        week_num=now.isocalendar()[1],
        deduplicated_count=deduplicated_count,
        wash_price_fn=wash_price,
        getattr=getattr,
    )
