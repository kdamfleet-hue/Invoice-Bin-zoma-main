...
{ href: '/', icon: '🏠', label: 'الرئيسية' },
    ...
    fetch('/api/system/my_permissions')
        ...
        .then(j => {
            ...
            // Filter Sidebar Navigation links if present
...
// ==========================================================================
// Fix: prevent dual menus — if enterprise aside.bz-sidebar exists, control it only.
// Otherwise fall back to legacy .bz-topbar drawer. UI-only; no data changes.
document.addEventListener('DOMContentLoaded', () => {
    const enterpriseSidebar = document.getElementById('bzSidebar') || document.querySelector('aside.bz-sidebar');
    const topbar = document.querySelector('.bz-topbar');

    // --- Case A: unified enterprise sidebar already in the page ---
    if (enterpriseSidebar) {
        // Never let topbar act as a second sidebar behind/alongside it
        if (topbar) {
            topbar.classList.remove('sidebar-open');
            document.body.classList.remove('sidebar-is-open');
        }

        let toggleBtn = document.getElementById('sidebarToggle') || document.querySelector('.bz-sidebar-toggle');
        if (!toggleBtn) {
        ...
        const contentWrapper = document.getElementById('content-wrapper') || document.querySelector('.bz-workspace-wrapper');

        const setOpen = (open) => {
            enterpriseSidebar.classList.toggle('collapsed', !open);
            document.body.classList.toggle('sidebar-is-open', open);
            if (contentWrapper) contentWrapper.classList.toggle('expanded', !open);
            if (open) {
                const w = enterpriseSidebar.offsetWidth || 280;
                document.documentElement.style.setProperty('--dynamic-sidebar-width', w + 'px');
            } else {
                document.documentElement.style.setProperty('--dynamic-sidebar-width', '0px');
            }
        };

        // Default: sidebar visible on desktop, collapsed on small screens
        const preferOpen = window.innerWidth >= 768;
        setOpen(preferOpen);

        toggleBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            const open = enterpriseSidebar.classList.contains('collapsed');
            setOpen(open);
        });

        document.addEventListener('click', (e) => {
            if (window.innerWidth < 768 && !enterpriseSidebar.classList.contains('collapsed')) {
                if (!enterpriseSidebar.contains(e.target) && !toggleBtn.contains(e.target)) {
                    setOpen(false);
                }
            }
        });
    ...
    // --- Case B: legacy pages with .bz-topbar only ---
    if (topbar) {
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Create Modal HTML
    const searchHTML = `
    <div id="quickSearchModal" class="quick-search-modal" style="display:none; opacity:0; transition: opacity 0.2s;">
        <div class="quick-search-backdrop" onclick="closeQuickSearch()"></div>
        <div class="quick-search-content">
            <div class="quick-search-header">
                <i data-lucide="search" style="color:var(--gold-primary); margin-left: 10px;"></i>
                <input type="text" id="quickSearchInput" placeholder="ابحث عن سائق، رقم لوحة، رقم وظيفي..." autocomplete="off" />
                <button class="quick-search-close" onclick="closeQuickSearch()"><i data-lucide="x"></i></button>
            </div>
            <div id="quickSearchResults" class="quick-search-results">
                <div class="qs-empty">ابدأ الكتابة للبحث...</div>
            </div>
        </div>
    </div>
    `;
    ...
    // 2. Listen for Keyboard Shortcut (Ctrl+K or Cmd+K)
    document.addEventListener('keydown', (e) => {
        if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
            e.preventDefault();
            openQuickSearch();
        }
        if (e.key === 'Escape' && qsModal.style.display === 'flex') {
            closeQuickSearch();
        }
    });
    ...
    window.openQuickSearch = function() {
        ...
        setTimeout(() => qsModal.style.opacity = '1', 10);
        qsInput.value = '';
        qsResults.innerHTML = '<div class="qs-empty">ابدأ الكتابة للبحث...</div>';
        qsInput.focus();
        document.body.style.overflow = 'hidden';
    };

    window.closeQuickSearch = function() {
        qsModal.style.opacity = '0';
        setTimeout(() => {
    ...
    qsInput.addEventListener('input', (e) => {
        ...
        if (query.length < 2) {
            qsResults.innerHTML = '<div class="qs-empty">اكتب حرفين على الأقل للبحث...</div>';
            return;
        }

        qsResults.innerHTML = '<div class="qs-loader"><i data-lucide="loader" class="spin"></i> جاري البحث...</div>';
        if(typeof lucide !== 'undefined') lucide.createIcons();

        searchTimeout = setTimeout(async () => {
            // Quick Search API call logic here...
            try {
                const res = await fetch(`/api/quick_search?q=${encodeURIComponent(query)}`);
                const data = await res.json();
                
                if (data.length === 0) {
                    qsResults.innerHTML = '<div class="qs-empty">لم يتم العثور على نتائج.</div>';
                    return;
                }

                qsResults.innerHTML = data.map(item => `
                    <a href="${_bzEscHtml(item.url)}" class="qs-result-item" onclick="closeQuickSearch()">
                        <div class="qs-icon">
                            <i data-lucide="${item.type === 'driver' ? 'steering-wheel' : 'user'}"></i>
                        </div>
                        <div class="qs-info">
                            <div class="qs-title">${_bzEscHtml(item.title)}</div>
                            <div class="qs-subtitle">${_bzEscHtml(item.subtitle)}</div>
                        </div>
                        <div class="qs-arrow"><i data-lucide="chevron-left"></i></div>
                    </a>
                `).join('');
                if(typeof lucide !== 'undefined') lucide.createIcons();
                ...
                qsResults.innerHTML = '<div class="qs-empty" style="color: #ef4444;">حدث خطأ أثناء البحث.</div>';
            }
        }, 300);
    });
});

// Note: a "live notifications dropdown" used to be built here, but it duplicated the
// already-working bell dropdown from setupNotifications() above — with the wrong field
// names for the real /api/notifications response (item.body/type/action_url don't exist;
// the real fields are title/kind/icon/branch/user/ts, exactly what setupNotifications()
// already uses correctly) — and it cloned+replaced #bzBell, which wiped out
// setupNotifications()'s working click handler in the process. Removed rather than
// patched, since a correct implementation already exists and this was pure duplication.
