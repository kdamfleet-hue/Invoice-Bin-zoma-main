...
{% block title %}سجل الإطارات | إدارة المخزون{% endblock %}
...
<div class="content-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 25px; padding-bottom: 15px; border-bottom: 2px solid var(--border-glass);">
    <div>
        <h2 style="margin: 0; color: var(--accent-primary); font-weight: 800; font-size: 1.8rem;"><i class="fas fa-compact-disc"></i> سجل الإطارات</h2>
        <p style="margin: 5px 0 0 0; color: var(--text-muted);">تتبع حركة الإطارات، تواريخ التركيب، وعمليات التدفئة.</p>
    </div>
    <button onclick="document.getElementById('addModal').style.display='block'" class="btn-primary" style="background: var(--accent-primary); color: #111; font-weight: bold; border: none; padding: 10px 20px; border-radius: 8px; cursor: pointer; box-shadow: 0 4px 10px rgba(197, 160, 89, 0.3); transition: all 0.3s;">
        <i class="fas fa-plus"></i> تسجيل إطار جديد
    </button>
</div>
    ...
    <table style="width: 100%; border-collapse: collapse; text-align: right;">
        <thead style="background: rgba(0,0,0,0.3); border-bottom: 1px solid var(--border-glass);">
            <tr>
                ...
                <th style="padding: 15px; color: var(--text-muted);">الماركة / المقاس</th>
                <th style="padding: 15px; color: var(--text-muted);">تاريخ التركيب</th>
                <th style="padding: 15px; color: var(--text-muted);">الحالة</th>
                <th style="padding: 15px; color: var(--text-muted);">تاريخ التدفئة</th>
            </tr>
        </thead>
        <tbody>
            {% for tire in tires %}
            <tr style="border-bottom: 1px solid var(--border-glass); transition: background 0.2s;">
                <td style="padding: 15px; font-weight: bold; color: white;">{{ tire.serial_number }}</td>
                <td style="padding: 15px; color: #ddd;">
                    {% if tire.vehicle_id %}
                        {% for v in vehicles %}
                            {% if v.id == tire.vehicle_id %}{{ v.plate_number }} - {{ v.model }}{% endif %}
                        {% endfor %}
                    {% else %}
                        <span style="color:var(--text-muted);">بالمستودع</span>
                    {% endif %}
                </td>
                <td style="padding: 15px; color: #ddd;">{{ tire.brand }} ({{ tire.size }})</td>
                <td style="padding: 15px; color: #ddd;">{{ tire.install_date or '—' }}</td>
                <td style="padding: 15px;">
                    <span style="padding: 4px 10px; border-radius: 20px; font-size: 0.8rem; background: {% if tire.status == 'جديد' %}rgba(34,197,94,0.2); color:#4ade80;{% elif tire.status == 'مستخدم' %}rgba(234,179,8,0.2); color:#facc15;{% else %}rgba(239,68,68,0.2); color:#f87171;{% endif %}">{{ tire.status }}</span>
                </td>
                <td style="padding: 15px; color: #ddd;">{{ tire.retread_date or '—' }}</td>
            </tr>
            {% else %}
            <tr>
                <td colspan="6" style="padding: 30px; text-align: center; color: var(--text-muted);">لا توجد إطارات مسجلة.</td>
            </tr>
            {% endfor %}
        </tbody>
    </table>
...
<div id="addModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:9999; backdrop-filter: blur(4px);">
    <div style="background: var(--bg-panel); border: 1px solid var(--border-glass); width: 450px; margin: 80px auto; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
        <div style="background: rgba(0,0,0,0.2); padding: 20px; border-bottom: 1px solid var(--border-glass); display: flex; justify-content: space-between; align-items: center;">
            <h3 style="margin: 0; color: var(--accent-primary);">تسجيل إطار جديد</h3>
        ...
        <form id="addForm" style="padding: 20px; display: flex; flex-direction: column; gap: 15px;">
            ...
            <div style="display: flex; gap: 10px;">
                ...
                <div style="flex:1;">
                    <label style="display:block; margin-bottom: 5px; color: var(--text-muted); font-size: 0.9rem;">المقاس</label>
                    <input type="text" id="size" style="width: 100%; padding: 10px; background: rgba(0,0,0,0.2); border: 1px solid var(--border-glass); border-radius: 6px; color: white;">
                </div>
            </div>
            <div style="display: flex; gap: 10px;">
                <div style="flex:1;">
...
document.getElementById('addForm').addEventListener('submit', async (e) => {
    ...
    try {
        const r = await fetch('/api/inventory/tires', {
            method: 'POST',
...
with open("templates/inventory_tires.html", "w", encoding="utf-8") as f:
    f.write(html)
print("Created inventory_tires.html")
